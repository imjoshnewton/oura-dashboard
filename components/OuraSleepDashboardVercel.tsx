...
import { MoonIcon, SunIcon, ActivityIcon, HeartIcon, ClockIcon, BarChart2Icon, CalendarIcon } from 'lucide-react'
...
const parseDurationToMinutes = (durationStr: string) => {
  const regex = /(?:(\d+) hours?,\s*)?(?:(\d+) minutes?(?:,\s*(\d+) seconds?)?)?/
...
export default function OuraSleepDashboardVercel({ sleepData, timeFrame = 7 }: OuraSleepDashboardVercelProps) {
  ...
  const totalSleepMinutes = parseDurationToMinutes(entry.total_sleep_duration)
  const deepSleepMinutes = parseDurationToMinutes(entry.deep_sleep_duration)
  const lightSleepMinutes = parseDurationToMinutes(entry.light_sleep_duration)
  ...
  const calculateAverages = () => {
    ...
    const avgSleep = processedData.reduce((sum, entry) => sum + entry.totalSleepMinutes, 0) / totalEntries
    const avgEfficiency = processedData.reduce((sum, entry) => sum + entry.efficiency, 0) / totalEntries
    const avgReadiness = processedData.reduce((sum, entry) => sum + entry.readinessScore, 0) / totalEntries
    const avgHr = processedData.reduce((sum, entry) => sum + entry.averageHeartRate, 0) / totalEntries
    const avgHrv = processedData.reduce((sum, entry) => sum + entry.averageHrv, 0) / totalEntries
...
const getReadinessLabel = (score: number) => {
  ...
  const [selectedDay, setSelectedDay] = useState(processedData[processedData.length - 1])
      ...
      <header className="sticky top-0 z-10 border-b bg-white dark:bg-gray-950 dark:border-gray-800">
        <div className="container flex items-center justify-between h-16 px-4 mx-auto">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold">Oura Sleep Dashboard (Vercel)</h1>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-6">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg. Sleep Duration</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center">
                <ClockIcon className="w-5 h-5 mr-2 text-purple-600" />
                <div className="text-2xl font-bold">{formatMinutes(averages.avgSleep)}</div>
              </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg. Sleep Efficiency</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg. Readiness</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center">
                <BarChart2Icon className="w-5 h-5 mr-2 text-amber-600" />
                <div className="text-2xl font-bold">{averages.avgReadiness}</div>
              </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-500 dark:text-gray-400">Avg. Heart Rate & HRV</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center">
              ...
              <p className="text-xs text-gray-500 mt-1">HRV: {averages.avgHrv} ms</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3 mb-6">
          <Card className="md:col-span-2">
            <CardHeader>
              ...
              <CardDescription>Total sleep time over the past week</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="grid gap-6 md:grid-cols-12">
          <Card className="md:col-span-8">
            <CardHeader>
              ...
              <CardDescription>Select a day to view detailed sleep information</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2 mb-6">
                {processedData.map((day) => (
                  <Button 
                    key={day.day}
                    variant={selectedDay.day === day.day ? "default" : "outline"}
                    size="sm"
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedDay.bedtimeStart}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Wake-up:</span>
                      <span className="font-medium">{selectedDay.bedtimeEnd}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Total Sleep:</span>
                      <span className="font-medium">{formatMinutes(selectedDay.totalSleepMinutes)}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center">
                        <span className="font-medium mr-2">{selectedDay.efficiency}%</span>
                        <Badge className={getSleepQualityLabel(selectedDay.efficiency).color}>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center">
                        <span className="font-medium mr-2">{selectedDay.readinessScore}</span>
                        <Badge className={getReadinessLabel(selectedDay.readinessScore).color}>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="font-medium">{formatMinutes(selectedDay.deepSleepMinutes)}</span>
                      </div>
                      <Progress 
                        value={(selectedDay.deepSleepMinutes / selectedDay.totalSleepMinutes) * 100} 
                    ...
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="font-medium">{formatMinutes(selectedDay.remSleepMinutes)}</span>
                      </div>
                      <Progress 
                        value={(selectedDay.remSleepMinutes / selectedDay.totalSleepMinutes) * 100} 
                        className="h-2 bg-gray-200" 
                      />
                    </div>
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="font-medium">{formatMinutes(selectedDay.lightSleepMinutes)}</span>
                      </div>
                      <Progress 
                        value={(selectedDay.lightSleepMinutes / selectedDay.totalSleepMinutes) * 100} 
                        className="h-2 bg-gray-200" 
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedDay.averageHeartRate} bpm</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Avg. HRV:</span>
                      <span className="font-medium">{selectedDay.averageHrv} ms</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <Card className="md:col-span-4">
            ...
            <CardContent>
              <div className="space-y-4">
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Sleep Duration Trend</h3>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Sleep Quality Distribution</h3>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-purple-100 dark:bg-purple-950 p-2 rounded-md">
                      <div className="text-lg font-bold text-purple-700 dark:text-purple-400">
                        {Math.round(processedData.reduce((sum, day) => sum + (day.deepSleepMinutes / day.totalSleepMinutes) * 100, 0) / processedData.length)}%
                      </div>
                      <div className="text-xs text-gray-500">Deep</div>
                    </div>
                    <div className="bg-purple-100 dark:bg-purple-950 p-2 rounded-md">
                      <div className="text-lg font-bold text-purple-700 dark:text-purple-400">
                        {Math.round(processedData.reduce((sum, day) => sum + (day.remSleepMinutes / day.totalSleepMinutes) * 100, 0) / processedData.length)}%
                      </div>
                      <div className="text-xs text-gray-500">REM</div>
                    </div>
                    <div className="bg-purple-100 dark:bg-purple-950 p-2 rounded-md">
                      <div className="text-lg font-bold text-purple-700 dark:text-purple-400">
                        {Math.round(processedData.reduce((sum, day) => sum + (day.lightSleepMinutes / day.totalSleepMinutes) * 100, 0) / processedData.length)}%
                      </div>
                ...
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-2">Sleep Consistency</h3>
                  <div className="space-y-2">
                    <div>
                ...
                <div>
                  ...
                  <ul className="space-y-2 text-sm">
                    <li className="flex items-start gap-2">
                      ...
                      <span>Try to maintain a more consistent bedtime schedule</span>
                    </li>
                    <li className="flex items-start gap-2">
